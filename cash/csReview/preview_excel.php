...
for ($i = 0; $i < count($_FILES['my_file']['name']); $i++) {
    ...
    if ($_FILES['my_file']['error'][$i] === UPLOAD_ERR_OK) {
        if (explode(".", $_FILES['my_file']['name'][$i])[1] !== "xlsx") {
            ...
            $check++;
            $python_script = "python ./preview_excel.py " . $fin_dest;
            $result = json_decode(exec($python_script), true);
            $html=$html."<li id='preview_".$check."' onclick='preview(".$check.");' class='preview_title'>".$_FILES['my_file']['name'][$i]."&nbsp;(僅部分資料)&nbsp;&nbsp;&nbsp;▼".$result."</li>";
        }
    } else {
